"""
Plot heatmaps for Adam baseline experiments: Learning Rate vs Batch Size.

This script creates 2D heatmap visualizations showing how Adam performance varies
across the learning rate and batch size grid.

Usage:
    python scripts/plot_adam_heatmap.py [--epoch EPOCH] [--metric METRIC]
"""
import argparse
...
def plot_adam_heatmap(
    experiments,
    metric_name="va_Accuracy",
    epoch=None,
):
    """
    Create heatmaps for Adam showing learning rate vs batch size.
    Creates separate plots for with and without momentum (beta1).
    ...
    for exp_w_data in exps_w_data:
        ...
        # Extract momentum (beta1 for Adam)
    ...
    exps_df = exps_df[exps_df["opt"] == "Adam"]
    print(f"\n=== DEBUG: After filtering for Adam ===")
    print(f"Rows remaining: {len(exps_df)}")
    
    if len(exps_df) == 0:
        print("\nERROR: No Adam experiments found!")
    ...
    for idx, (momentum_value, variant_name) in enumerate([(0.0, "without Momentum"), (0.9, "with Momentum")]):
        print(f"\n=== DEBUG: Processing {variant_name} (beta1={momentum_value}) ===")
        ...
        if momentum_df.empty:
            ...
            axes[idx].set_title(f"Adam {variant_name}")
            continue
        ...
        ax.set_title(f"Adam {variant_name}")
    ...
    plt.suptitle(
        f"Adam Baseline: {metric_name} at Epoch {epoch}",
        fontsize=14,
        y=0.98
    )
    plt.tight_layout()
    ...
    filename = f"Adam_{metric_name}_epoch{epoch}.png"
...
def plot_all_adam_metrics(experiments, epoch=None):
    """Plot Adam heatmaps for all key metrics."""
    metrics = [
        "va_Accuracy",
        "tr_Accuracy", 
        "va_CrossEntropyLoss",
    ...
    for metric in metrics:
        print(f"\n{'='*80}")
        print(f"Plotting {metric} for Adam...")
        print(f"{'='*80}")
        try:
            plot_adam_heatmap(
                experiments,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot Adam baseline heatmaps")
    parser.add_argument(
        "--epoch",
        type=int,
        default=None,
    ...
    if args.metric:
        ...
        # Plot all metrics
        plot_all_adam_metrics(exps_to_plot, epoch=args.epoch)
